...
$title = $langs->trans("CodeZone")." - ".$langs->trans('Card');
//$title = $object->ref." - ".$langs->trans('Card');
if ($action == 'create') {
	$title = $langs->trans("NewObject", $langs->transnoentitiesnoconv("CodeZone"));
}
$help_url = '';
...
if ($action == 'create') {
	...
	foreach ($object->fields as $key => $val) {
		...
		if (!in_array(abs($val['visible']), [1, 3, 4]) || (isset($val['enabled']) && !verifCond($val['enabled']))) {
			continue;
		}
		...
		// Vérifier si le champ est celui du mot de passe basé sur son étiquette
		if ($key == 'pass_crypted' && $val['label'] == 'Password') {
			$generated_password = getRandomPassword(false);
			$password = (GETPOSTISSET('password') ? GETPOST('password') : $generated_password);
			$valuetoshow = '<input class="minwidth300 maxwidth400 widthcentpercentminusx" maxlength="128" type="text" id="password" name="password" value="'.dol_escape_htmltag($password).'" autocomplete="new-password">';
...
if (($id || $ref) && $action == 'edit') {
	print load_fiche_titre($langs->trans("CodeZone"), '', 'object_'.$object->picto);
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	$head = codezonePrepareHead($object);

	print dol_get_fiche_head($head, 'card', $langs->trans("CodeZone"), -1, $object->picto, 0, '', '', 0, '', 1);

	$formconfirm = '';

	// Confirmation to delete (using preloaded confirm popup)
	if ($action == 'delete' || ($conf->use_javascript_ajax && empty($conf->dol_use_jmobile))) {
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"].'?id='.$object->id, $langs->trans('DeleteCodeZone'), $langs->trans('ConfirmDeleteObject'), 'confirm_delete', '', 0, 'action-delete');
	}
	...
	if ($action == 'validate') {
		...
		$text = $langs->trans('ConfirmActionCodeZone', $numref);

		if (!empty($conf->notification->enabled)) {
			require_once DOL_DOCUMENT_ROOT . '/core/class/notify.class.php';
			$notify = new Notify($db);
			$text .= '<br>';
			$text .= $notify->confirmMessage('CODEZONE_VALIDATE', $object->socid, $object);
		}

		if (!$error) {
			$formquestion = array();
			$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"] . '?id=' . $object->id, $langs->trans('ValidateCodeZone'), $text, 'confirm_validate', $formquestion, 0, 1, 220);
		}
	...
	*/
	...
	if ($action != 'presend' && $action != 'editline') {
		...
		if (empty($reshook)) {
			...
			// Validate
			if ($object->status == CodeZone::STATUS_DRAFT) {
	...
	if ($action != 'presend') {
		...
		$linktoelem = $form->showLinkToObjectBlock($object, null, array('codezone'));
