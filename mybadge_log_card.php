...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('sigrebadge', 'mybadge_log', 'write'); // Used by the include of actions_addupdatedelete.inc.php and actions_lineupdown.inc.php
	$permissiontodelete = $user->hasRight('sigrebadge', 'mybadge_log', 'delete') || ($permissiontoadd && isset($object->status) && $object->status == $object::STATUS_DRAFT);
	$permissionnote = $user->hasRight('sigrebadge', 'mybadge_log', 'write'); // Used by the include of actions_setnotes.inc.php
	$permissiondellink = $user->hasRight('sigrebadge', 'mybadge_log', 'write'); // Used by the include of actions_dellink.inc.php
} else {
	$permissiontoread = 1;
...
$upload_dir = $conf->sigrebadge->multidir_output[isset($object->entity) ? $object->entity : 1].'/mybadge_log';
...
if (empty($reshook)) {
	...
	$backurlforlist = dol_buildpath('/sigrebadge/mybadge_log_list.php', 1);
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
				...
				$backtopage = dol_buildpath('/sigrebadge/mybadge_log_card.php', 1).'?id='.((!empty($id) && $id > 0) ? $id : '__ID__');
			}
		}
	}
	...
	*/
	...
	if ($action != 'presend') {
		...
		if ($includedocgeneration) {
			...
			print $formfile->showdocuments('sigrebadge:Mybadge_log', $object->element.'/'.$objref, $filedir, $urlsource, $genallowed, $delallowed, $object->model_pdf, 1, 0, 0, 28, 0, '', '', '', $langs->defaultlang);
		}
		...
		$morehtmlcenter = dolGetButtonTitle($langs->trans('SeeAll'), '', 'fa fa-bars imgforviewmode', dol_buildpath('/sigrebadge/mybadge_log_agenda.php', 1).'?id='.$object->id);
