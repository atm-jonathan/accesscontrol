...
/**
 *    \file       mybadge_log_card.php
 *    \ingroup    accesscontrol
 *    \brief      Page to create/edit/view mybadge_log
 */
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('accesscontrol', 'mybadge_log', 'write'); // Used by the include of actions_addupdatedelete.inc.php and actions_lineupdown.inc.php
	$permissiontodelete = $user->hasRight('accesscontrol', 'mybadge_log', 'delete') || ($permissiontoadd && isset($object->status) && $object->status == $object::STATUS_DRAFT);
	$permissionnote = $user->hasRight('accesscontrol', 'mybadge_log', 'write'); // Used by the include of actions_setnotes.inc.php
	$permissiondellink = $user->hasRight('accesscontrol', 'mybadge_log', 'write'); // Used by the include of actions_dellink.inc.php
} else {
	$permissiontoread = 1;
...
$upload_dir = $conf->accesscontrol->multidir_output[isset($object->entity) ? $object->entity : 1].'/mybadge_log';
...
if (empty($reshook)) {
	...
	$backurlforlist = dol_buildpath('/accesscontrol/mybadge_log_list.php', 1);
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
				...
				$backtopage = dol_buildpath('/accesscontrol/mybadge_log_card.php', 1).'?id='.((!empty($id) && $id > 0) ? $id : '__ID__');
			}
		}
	}
...
$title = $langs->trans("Mybadge_log")." - ".$langs->trans('Card');
//$title = $object->ref." - ".$langs->trans('Card');
if ($action == 'create') {
	$title = $langs->trans("NewObject", $langs->transnoentitiesnoconv("Mybadge_log"));
}
$help_url = '';
...
if (($id || $ref) && $action == 'edit') {
	print load_fiche_titre($langs->trans("Mybadge_log"), '', 'object_'.$object->picto);
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	$head = mybadge_logPrepareHead($object);

	print dol_get_fiche_head($head, 'card', $langs->trans("Mybadge_log"), -1, $object->picto, 0, '', '', 0, '', 1);

	$formconfirm = '';

	// Confirmation to delete (using preloaded confirm popup)
	if ($action == 'delete' || ($conf->use_javascript_ajax && empty($conf->dol_use_jmobile))) {
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"].'?id='.$object->id, $langs->trans('DeleteMybadge_log'), $langs->trans('ConfirmDeleteObject'), 'confirm_delete', '', 0, 'action-delete');
	}
	...
	if ($action == 'xxx') {
		$text = $langs->trans('ConfirmActionMybadge_log', $object->ref);
		/*if (isModEnabled('notification'))
		{
			require_once DOL_DOCUMENT_ROOT . '/core/class/notify.class.php';
			$notify = new Notify($db);
			$text .= '<br>';
			$text .= $notify->confirmMessage('MYBADGE_CLOSE', $object->socid, $object);
		}*/
	...
	*/
	...
	if ($action != 'presend') {
		...
		if ($includedocgeneration) {
			...
			print $formfile->showdocuments('accesscontrol:Mybadge_log', $object->element.'/'.$objref, $filedir, $urlsource, $genallowed, $delallowed, $object->model_pdf, 1, 0, 0, 28, 0, '', '', '', $langs->defaultlang);
		}

		// Show links to link elements
		$linktoelem = $form->showLinkToObjectBlock($object, null, array('mybadge_log'));
		...
		$morehtmlcenter = dolGetButtonTitle($langs->trans('SeeAll'), '', 'fa fa-bars imgforviewmode', dol_buildpath('/accesscontrol/mybadge_log_agenda.php', 1).'?id='.$object->id);
