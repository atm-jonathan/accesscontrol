...
if (isModEnabled('sigrebadge') && $user->hasRight('sigrebadge', 'read')) {
	...
	if ($resql)
	{
		...
		print '<th colspan="3">'.$langs->trans("DraftMyBadges").($num?'<span class="badge marginleftonlyshort">'.$num.'</span>':'').'</th></tr>';
		...
		if ($num > 0)
		{
			...
			while ($i < $num)
			{
				...
				$mybadgestatic->id=$obj->rowid;
				$mybadgestatic->ref=$obj->ref;
				$mybadgestatic->ref_client=$obj->ref_client;
				$mybadgestatic->total_ht = $obj->total_ht;
				$mybadgestatic->total_tva = $obj->total_tva;
				$mybadgestatic->total_ttc = $obj->total_ttc;

				print $mybadgestatic->getNomUrl(1);
				print '</td>';
...
END MODULEBUILDER DRAFT MYBADGE */
...
if (isModEnabled('sigrebadge') && $user->hasRight('sigrebadge', 'read')) {
	...
	if ($resql)
	{
		...
		print '<tr class="liste_titre">';
		print '<th colspan="2">';
		print $langs->trans("BoxTitleLatestModifiedMyBadges", $max);
		print '</th>';
		...
		if ($num)
		{
			while ($i < $num)
			{
				...
				$mybadgestatic->id=$objp->rowid;
				$mybadgestatic->ref=$objp->ref;
				$mybadgestatic->label=$objp->label;
				$mybadgestatic->status = $objp->status;

				print '<tr class="oddeven">';
				print '<td class="nowrap">'.$mybadgestatic->getNomUrl(1).'</td>';
