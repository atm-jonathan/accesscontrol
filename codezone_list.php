...
/**
 *   	\file       htdocs/modulebuilder/template/codezone_list.php
 *		\ingroup    accesscontrol
 *		\brief      List page for codezone
 */
...
require_once __DIR__.'/class/codezone.class.php';
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('accesscontrol', 'codezone', 'write');
	$permissiontodelete = $user->hasRight('accesscontrol', 'codezone', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'CodeZone';
	$objectlabel = 'CodeZone';
...
$title = $langs->trans("CodeZones");
//$help_url = "EN:Module_CodeZone|FR:Module_CodeZone_FR|ES:Módulo_CodeZone";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/accesscontrol/codezone_card.php', 1).'?id='.((int) $id));
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/accesscontrol/codezone_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendCodeZoneRef";
$modelmail = "codezone";
$objecttmp = new CodeZone($db);
...
if ($search_all) {
	...
	print '<!-- Search done like if CODEZONE_QUICKSEARCH_ON_FIELDS = '.$setupstring.' -->'."\n";
