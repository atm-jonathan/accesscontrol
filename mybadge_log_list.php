...
/**
 *   	\file       mybadge_log_list.php
 *		\ingroup    accesscontrol
 *		\brief      List page for mybadge_log
 */
...
require_once __DIR__.'/class/mybadge_log.class.php';
...
if ($enablepermissioncheck) {
	...
	$permissiontodelete = $user->hasRight('accesscontrol', 'mybadge_log', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'Mybadge_log';
	$objectlabel = 'Mybadge_log';
...
$title = $langs->trans("Mybadge_logs");
//$help_url = "EN:Module_Mybadge_log|FR:Module_Mybadge_log_FR|ES:Módulo_Mybadge_log";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/accesscontrol/mybadge_log_card.php', 1).'?id='.((int) $id));
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/accesscontrol/mybadge_log_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendMybadge_logRef";
$modelmail = "mybadge_log";
$objecttmp = new Mybadge_log($db);
...
if ($search_all) {
	...
	print '<!-- Search done like if MYBADGE_QUICKSEARCH_ON_FIELDS = '.$setupstring.' -->'."\n";
