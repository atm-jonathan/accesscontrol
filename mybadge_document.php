...
/**
 *  \file       mybadge_document.php
 *  \ingroup    accesscontrol
 *  \brief      Tab for documents linked to MyBadge
 */
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->accesscontrol->multidir_output[$object->entity ? $object->entity : $conf->entity]."/mybadge/".get_exdir(0, 0, 0, 1, $object);
}
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd  = $user->hasRight('accesscontrol', 'mybadge', 'write'); // Used by the include of actions_addupdatedelete.inc.php and actions_linkedfiles.inc.php
} else {
	$permissiontoread = 1;
	$permissiontoadd  = 1;
}
...
// Header
// ------
$title = $langs->trans("MyBadge")." - ".$langs->trans("Files");
...
print dol_get_fiche_head($head, 'document', $langs->trans("MyBadge"), -1, $object->picto);
...
$param = '&id='.$object->id;
//$relativepathwithnofile='mybadge/' . dol_sanitizeFileName($object->id).'/';
$relativepathwithnofile = 'mybadge/'.dol_sanitizeFileName($object->ref).'/';
