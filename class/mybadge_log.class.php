...
/**
 * Class for Mybadge_log
 */
class Mybadge_log extends CommonObject
{
	/**
	...
	public $fields=array(
		...
		"fk_mybadge" => array("type"=>"integer", "label"=>"Fkmybadge", "enabled"=>"1", 'position'=>15, 'notnull'=>1, "visible"=>"-1", "css"=>"maxwidth500 widthcentpercentminusxx",),
		...
		"fk_object" => array("type"=>"varchar(32)", "label"=>"Fkobject", "enabled"=>"1", 'position'=>35, 'notnull'=>0, "visible"=>"-1", "css"=>"maxwidth500 widthcentpercentminusxx",),
		"fk_objectid" => array("type"=>"integer", "label"=>"Fkobjectid", "enabled"=>"1", 'position'=>40, 'notnull'=>0, "visible"=>"-1", "css"=>"maxwidth500 widthcentpercentminusxx",),
		"tms" => array("type"=>"timestamp", "label"=>"DateModification", "enabled"=>"1", 'position'=>45, 'notnull'=>1, "visible"=>"-1",),
		"comment" => array("type"=>"varchar(128)", "label"=>"Comment", "enabled"=>"1", 'position'=>50, 'notnull'=>0, "visible"=>"1",),
	);
	public $rowid;
	public $fk_mybadge;
	public $fk_user;
	...
	//  */
	// public $fk_element = 'fk_mybadge_log';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'Mybadge_logline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'Mybadge_log', 'fk_element'=>'fk_mybadge_log'));
	...
	public function validate($user, $notrigger = 0)
	{
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'mybadge_log/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'mybadge_log/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'mybadge_log/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'mybadge_log/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowMybadge_log")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("Mybadge_log").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		if (empty($notooltip)) {
			if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
				$label = $langs->trans("ShowMybadge_log");
	...
	public function getLinesArray()
	{
		...
		$objectline = new Mybadge_logLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_mybadge_log = '.((int) $this->id)));
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			...
			} elseif (getDolGlobalString('MYBADGE_ADDON_PDF')) {
				$modele = getDolGlobalString('MYBADGE_ADDON_PDF');
			}
		}
...
/**
 * Class Mybadge_logLine. You can also remove this and generate a CRUD class for lines objects.
 */
class Mybadge_logLine extends CommonObjectLine
{
	// To complete with content of an object Mybadge_logLine
	// We should have a field rowid, fk_mybadge_log and position
