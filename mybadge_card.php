...
$title = $langs->trans("MyBadge")." - ".$langs->trans('Card');
//$title = $object->ref." - ".$langs->trans('Card');
if ($action == 'create') {
	$title = $langs->trans("NewObject", $langs->transnoentitiesnoconv("MyBadge"));
}
$help_url = '';
...
if (($id || $ref) && $action == 'edit') {
	print load_fiche_titre($langs->trans("MyBadge"), '', 'object_'.$object->picto);
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	$head = mybadgePrepareHead($object);

	print dol_get_fiche_head($head, 'card', $langs->trans("MyBadge"), -1, $object->picto, 0, '', '', 0, '', 1);

	$formconfirm = '';

	// Confirmation to delete (using preloaded confirm popup)
	if ($action == 'delete' || ($conf->use_javascript_ajax && empty($conf->dol_use_jmobile))) {
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"].'?id='.$object->id, $langs->trans('DeleteMyBadge'), $langs->trans('ConfirmDeleteObject'), 'confirm_delete', '', 0, 'action-delete');
	}
	...
	if ($action == 'validate') {
		...
		$text = $langs->trans('ConfirmValidateMyBadge', $numref);

		if (!empty($conf->notification->enabled)) {
			require_once DOL_DOCUMENT_ROOT . '/core/class/notify.class.php';
			$notify = new Notify($db);
			$text .= '<br>';
			$text .= $notify->confirmMessage('MYBADGE_VALIDATE', $object->socid, $object);
		}

		if (!$error) {
			$formquestion = array();
			$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"] . '?id=' . $object->id, $langs->trans('ValidateMyBadge'), $text, 'confirm_validate', $formquestion, 0, 1, 220);
		}
	
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"].'?id='.$object->id, $langs->trans('ValidateBadge'), $text, 'confirm_validate', $formquestion, 0, 1, 220);
	}
	...
	*/
	...
	if ($action != 'presend' && $action != 'editline') {
		...
		if (empty($reshook)) {
			...
			// Validate
			if ($object->status == MyBadge::STATUS_DRAFT) {
	...
	if ($action != 'presend') {
		...
		$linktoelem = $form->showLinkToObjectBlock($object, null, array('mybadge'));
